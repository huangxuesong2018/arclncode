...
public class UserCoreServiceImpl implements IUserCoreService {
    ...
    private void beforeRegisterValidate(UserRegisterRequest request){
        ...
        if(com.alibaba.dubbo.common.utils.StringUtils.isEmpty(request.getUsername())){
            throw new ValidateException("用户名为空");
        }
        if(com.alibaba.dubbo.common.utils.StringUtils.isEmpty(request.getPassword())){
            throw new ValidateException("密码为空");
        }
        if(com.alibaba.dubbo.common.utils.StringUtils.isEmpty(request.getMobile())){
            throw new ValidateException("密码为空");
        }
    }
