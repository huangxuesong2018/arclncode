...
public class KafkaProducerDemo extends Thread{
    ...
    public KafkaProducerDemo(String topic,boolean isAysnc) {
        ...
        properties.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG,"192.168.1.130:9092,192.168.1.131:9092");
    ...
    public void run() {
        ...
        while (num < 50){
            ...
            System.out.println("besing send message :"+message);
            if(this.isAysnc){
                producer.send(new ProducerRecord<Integer, String>(topic, message), new Callback() {
                    @Override
                    public void onCompletion(RecordMetadata recordMetadata, Exception e) {
                        if(recordMetadata != null){
                            System.out.println("async-offset:"+recordMetadata.offset()+"->"+recordMetadata.partition());
                        }
                    }
                });
            }else{
                ...
                try {
                    ...
                    System.out.println("async-offset:"+metadata.offset()+"->"+metadata.partition());
